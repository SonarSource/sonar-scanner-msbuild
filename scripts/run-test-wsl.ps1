...
if (-Not $env:WSL_DISTRO_NAME) {
    ...
    wsl env ORCHESTRATOR_HOME=$orchestratorHome `
        ...
        SONARCLOUD_PROJECT_TOKEN=$env:SONARCLOUD_PROJECT_TOKEN `
        SONARCLOUD_URL=$env:SONARCLOUD_URL `
        SONARCLOUD_API_URL=$env:SONARCLOUD_API_URL `
        SONARCLOUD_ORGANIZATION=$env:SONARCLOUD_ORGANIZATION `
