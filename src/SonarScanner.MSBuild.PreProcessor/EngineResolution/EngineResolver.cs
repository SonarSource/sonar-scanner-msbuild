...
public class EngineResolver : IResolver
{
    ...
    public async Task<string> ResolvePath(ProcessedArgs args)
    {
        ...
        if (args.EngineJarPath is { } localEngine)
        {
            ...
            runtime.Telemetry.AddTelemetryMessage(TelemetryKeys.NewBootstrappingEnabled, TelemetryValues.NewBootstrapping.Disabled);
            runtime.Telemetry.AddTelemetryMessage(TelemetryKeys.ScannerEngineDownload, TelemetryValues.ScannerEngineDownload.UserSupplied);
            return localEngine;
        }
        if (!server.SupportsJreProvisioning) // JRE and sonar engine provisioning were introduced by the same version of SQ Server
        {
            ...
            runtime.Telemetry.AddTelemetryMessage(TelemetryKeys.NewBootstrappingEnabled, TelemetryValues.NewBootstrapping.Unsupported);
            return null;
        }
        runtime.Telemetry.AddTelemetryMessage(TelemetryKeys.NewBootstrappingEnabled, TelemetryValues.NewBootstrapping.Enabled);
    ...
    private async Task<string> ResolveEnginePath(EngineMetadata metadata)
    {
        ...
        switch (await downloader.DownloadFileAsync(() => server.DownloadEngineAsync(metadata)))
        {
            case Downloaded success:
                ...
                runtime.Telemetry.AddTelemetryMessage(TelemetryKeys.ScannerEngineDownload, TelemetryValues.ScannerEngineDownload.Downloaded);
                return success.FilePath;
            case CacheHit cacheHit:
                runtime.LogDebug(Resources.MSG_Resolver_CacheHit, nameof(EngineResolver), cacheHit.FilePath);
                runtime.Telemetry.AddTelemetryMessage(TelemetryKeys.ScannerEngineDownload, TelemetryValues.ScannerEngineDownload.CacheHit);
                return cacheHit.FilePath;
            case DownloadError error:
                runtime.LogDebug(Resources.MSG_Resolver_DownloadFailure, nameof(EngineResolver), error.Message);
                runtime.Telemetry.AddTelemetryMessage(TelemetryKeys.ScannerEngineDownload, TelemetryValues.ScannerEngineDownload.Failed);
                return null;
            default:
