...
public static class TelemetryUtils
{
    ...
    {
        if (properties.FirstOrDefault(x => x.Id.Equals(SonarProperties.ScanAllAnalysis)) is { } argument)
        {
            logger.AddTelemetryMessage("s4net.params.sonar_scanner_scanAll.value", argument.Value);
            logger.AddTelemetryMessage("s4net.params.sonar_scanner_scanAll.value", provider.ToString());
        }
    }
}
