...
namespace SonarScanner.MSBuild.PreProcessor.WebServer
{
    public abstract class SonarWebServer : ISonarWebServer
    {
        ...
        public async Task<JreMetadata> DownloadJreMetadata(string operatingSystem, string architecture)
        {
            ...
            try
            {
                ...
                var jres = await apiDownloader.Download(uri);
                return JsonConvert.DeserializeObject<IEnumerable<JreMetadata>>(jres).FirstOrDefault();
            }
            catch (Exception)
            {
                return null;
