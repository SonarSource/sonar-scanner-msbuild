...
namespace SonarScanner.MSBuild.Tasks
{
    ...
    public class GetAnalyzerSettings : Task
    {
        ...
        private static string GetMergedRuleSetContent(string originalRuleset, string languageRuleset) =>
            ...
            // * the first ruleset is the one specified by the user. We're inheriting the severities from
            //   that ruleset, so any "error" issues will fail the build.
...
<RuleSet Name=""RulesetName"" ToolsVersion=""14.0"">
  <Include Path=""{originalRuleset}"" Action=""Default"" />
