...
public class OrchestratorState {
  ...
  private void analyzeEmptyProject() throws Exception {
    ...
    assertTrue(TestUtils.newScannerBegin(ORCHESTRATOR, "Empty", projectDir, token, ScannerClassifier.NET_FRAMEWORK)
        .setDebugLogs(true) // FIXME: REMOVE DEBUG
        .execute(ORCHESTRATOR).isSuccess(),
      "Orchestrator warmup failed - begin step");
    TestUtils.runMSBuild(ORCHESTRATOR, projectDir, "/t:Restore,Rebuild", "/v:diag"); // FIXME: REMOVE DEBUG verbosity, use buildMSBuild instead
