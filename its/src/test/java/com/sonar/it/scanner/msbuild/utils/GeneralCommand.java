...
public class GeneralCommand extends BaseCommand<GeneralCommand> {
  ...
  private GeneralCommand(Command command) {
    super(Paths.get(""));
    this.command = command;
  }

  public static GeneralCommand create(Command command) {
    return new GeneralCommand(command);
  }
  ...
  public BuildResult execute() {
    ...
    if (returnCode == 0)
    {
      LOG.info("Command line success: {}", commandLine);
    } else
    {
      LOG.error("Command line failure: '{}'\nReturn code: {}\nLogs:{}", commandLine, returnCode, result.getLogs());
    }
    return result;
  }
