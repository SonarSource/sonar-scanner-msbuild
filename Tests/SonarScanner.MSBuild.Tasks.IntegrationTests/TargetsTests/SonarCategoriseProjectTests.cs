...
";
            ...
            // Assert
            AssertIsTestProject(result);
            AssertProjectIsExcluded(result, "Sonar: (foo.proj) project is a temporary project generated by Microsoft Fakes and will be ignored.");
        }
...
";
        ...
        public void WpfTemporaryProjects_AreExcluded()
        {
            ...
            void ExecuteTest(string projectName, bool expectedExclusionState)
            {
                ...
                {
                    AssertProjectIsExcluded(result, $"Sonar: ({projectName}) project is a temporary project and will be excluded.");
                }
                else
                {
                    AssertProjectIsNotExcluded(result);
        ...
        private static void AssertProjectIsExcluded(BuildLog log, string expectedLogMessage)
        {
            log.GetPropertyAsBoolean(TargetProperties.SonarQubeExcludeMetadata).Should().BeTrue();
            log.Messages.Should().Contain(expectedLogMessage);
        }
