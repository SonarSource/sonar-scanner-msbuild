...
public class CacheProcessorTests
{
    ...
    {
        using var sut = new CacheProcessor(client, CreateProcessedArgs("/k:key /d:sonar.pullrequest.base=master"), BuildSettings.CreateForTesting(), logger);
    ...
    private CacheProcessor CreateSut(BuildSettings buildSettings = null) =>
        new(client, CreateProcessedArgs(), buildSettings ?? BuildSettings.CreateForTesting(), logger);
