...
namespace SonarScanner.MSBuild.PreProcessor.Test
{
    ...
    {
        private readonly TestJavaVersion javaVersion = new("17.0.6.0");
        ...
        public void Execute_NullArguments_ThrowsArgumentNullException()
        {
            ...
            var preProcessor = new PreProcessor(factory, javaVersion, factory.Logger);
        ...
        {
            var factory = new MockObjectFactory();
            var sut = new PreProcessor(factory, javaVersion, factory.Logger);
...
@$"Failed to create an empty directory '{configDirectory}'. Please check that there are no open or read-only files in the directory and that you have the necessary read/write permissions.
  Detailed error message: The process cannot access the file 'LockedFile.txt' because it is being used by another process.");
        ...
        public async Task Execute_ServerNotAvailable_ReturnsFalse()
        {
            ...
            var preProcessor = new PreProcessor(factory, javaVersion, new TestLogger());
