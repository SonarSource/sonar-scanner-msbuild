...
namespace System.Security.Cryptography.X509Certificates
{
    public sealed partial class CertificateRevocationListBuilder
    {
        ...
        {
            _ = issuerCertificate ?? throw new ArgumentNullException(nameof(issuerCertificate));

            if (!issuerCertificate.HasPrivateKey)
                throw new ArgumentException(
                    "Cryptography_CertReq_IssuerRequiresPrivateKey",
                    nameof(issuerCertificate));
            if (nextUpdate <= thisUpdate)
                throw new ArgumentException("Cryptography_CRLBuilder_DatesReversed");
            ...
            {
                throw new ArgumentException(
                    "Cryptography_CertReq_BasicConstraintsRequired",
                    nameof(issuerCertificate));
            }
            ...
            {
                throw new ArgumentException(
                    "Cryptography_CertReq_IssuerBasicConstraintsInvalid",
                    nameof(issuerCertificate));
            }
            ...
            {
                throw new ArgumentException(
                    "Cryptography_CRLBuilder_IssuerKeyUsageInvalid",
                    nameof(issuerCertificate));
            }
            ...
            try
            {
                switch (keyAlgorithm)
                {
                    case Oids.Rsa:
                        ...
                        {
                            throw new ArgumentException("Cryptography_CertReq_RSAPaddingRequired");
                        }
                    ...
                    default:
                        throw new ArgumentException(
                            string.Format("Cryptography_UnknownKeyAlgorithm {0}", keyAlgorithm),
                            nameof(issuerCertificate));
                }
            ...
            X509AuthorityKeyIdentifierExtension authorityKeyIdentifier)
        {
            ...
            _ = authorityKeyIdentifier ?? throw new ArgumentNullException(nameof(authorityKeyIdentifier));
